...
import { formatMoviePremiereDate, isFutureComingSoonMovie, getMoviePremiereDateKey } from '../utils/catalogue-lifecycle';
...
export const HomePage: React.FC<HomePageProps> = ({ onNavigate }) => {
  ...
  const discovery = MOVIES.filter((movie) => !comingSoonIds.has(movie.id)).slice(0, 6);
  return (
    <div className="pt-0 pb-6 sm:pb-8" id="home-view">
      <PopularActorsSection onNavigate={onNavigate} />
      ...
      <section className="py-7 sm:py-9 border-b border-[#E7DFD5]" aria-labelledby="discovery-heading">
        <div className="flex items-end justify-between gap-4 mb-5">
          ...
          <NavigationLink href={getMoviesPath()} onNavigate={onNavigate} className="shrink-0 text-xs sm:text-sm font-sans-clean font-medium text-[#1A3D2F] hover:text-[#143626]">View all {MOVIES.length} movies <span className="xmas-nav-arrow">→</span></NavigationLink>
        </div>
        <div className="flex gap-4 sm:gap-5 overflow-x-auto pb-2 no-scrollbar snap-x snap-mandatory sm:grid sm:grid-cols-3 md:grid-cols-6 sm:overflow-visible">
